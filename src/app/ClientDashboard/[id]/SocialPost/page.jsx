// app/ClientDashboard/SocialPosts/page.jsx
"use client";
...
export default function SocialPostsBook() {
  ...
  const getPost = useCallback(async () => {
    try {
      ...
      // after React paints, tell pageflip to rescan
  ...
  const deletePost = async (postId) => {
    try {
      const res = await fetch(
        `${API_BASE}/api/executives/executive-social-posts/${postId}/`,
        {
          method: "DELETE",
          headers: { Authorization: `Token ${token}` },
        }
      ...
      setPosts1((prev) => prev.filter((r) => r.id !== postId));
      refreshBook();
    } catch (err) {
  ...
  const openEdit = (post) => {
    setEditPost(post);
    setEditPlatform((post.platform || "").toLowerCase());
    setEditUrl(post.url || "");
        ...
        {grouped.length === 0 ? (
          ...
          grouped.map(({ platform, items }) => (
            <div key={platform} className="mb-3 mt-3">
              <h3 className="text-lg md:text-xl font-black text-center my-3 font-semibold">
                {platform.toUpperCase()}
              </h3>
              <ol className="list-decimal pl-6 space-y-2">
                {items.map((p) => (
                  <li key={p.id} className="leading-relaxed hover:text-[#28d7a2]">
                    {p.caption}
                  </li>
                ))}
              </ol>
    ...
    return (
      <div className="p-6 bg-white h-full flex flex-col">
        <div className="flex items-center gap-6 mb-8">
          <span className="text-base uppercase text-gray-600 font-semibold">
            {platform}
          </span>
          <div className="flex gap-2 items-center justify-center">
            <button
              onClick={() => openEdit(post)}
              className="flex items-center gap-2 bg-yellow-400 hover:bg-yellow-500 text-black font-semibold px-2 py-1 rounded cursor-pointer"
            >
            ...
            <button
              ...
              className="flex items-center gap-2 bg-red-600 hover:bg-red-700 text-white font-semibold px-2 py-1 rounded cursor-pointer"
            >
        ...
        <h3 className="text-xl md:text-3xl font-bold mb-3 text-[#212529]">{caption}</h3>
  ...
  return (
    <div className="mx-auto my-5 w-[95vw] max-w-[1700px]">
      ...
      <div className="flex justify-between items-center mb-3">
        <p className="text-base">Dashboard / Social Post</p>
        <button
          onClick={() => setShowModal(true)}
          className="px-4 py-2 rounded bg-[#29d7a1] text-white hover:bg-emerald-600"
        >
          + Add Social Post
        </button>
      </div>
