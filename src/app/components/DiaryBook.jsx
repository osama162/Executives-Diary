        ...
        {tabs.map((t, i) => {
          ...
          return (
            <button
              ...
              disabled={isDisabled}
              className={[
                "text-[12px] px-[7px] py-[11px] text-center font-cinzel shadow-[1px_1px_7px_1px_#adadad] h-full flex flex-wrap items-center justify-center w-[33%] border",
                i === active
                  ? "bg-white text-black font-semibold border-transparent"
                  : "bg-white text-black ",
                isDisabled
...
function SectionHeading({ heading, sub }) {
  return (
    <>
      <h3 className="font-semibold text-[#212529] border-b-4 border-double pb-2 font-base">
        {heading}
      </h3>
      {sub ? (
        <h2 className="text-xl font-bold text-[#212529] mt-8 leading-6 capitalize font-base">
          {sub}
        </h2>
      ) : null}
    </>
      ...
      case "linkedin":
        return (
          <svg viewBox="0 0 24 24" width="28" height="28" fill="#0a66c2">
            <path d="M4.98 3.5C4.98 4.88 3.87 6 2.5 6S0 4.88 0 3.5 1.12 1 2.5 1s2.48 1.12 2.48 2.5zM.5 8.5h4V23h-4V8.5zM8 8.5h3.8v2h.05c.53-1 1.83-2.05 3.77-2.05C19.6 8.45 22 10.3 22 14.13V23h-4v-7.4c0-1.76-.03-4.02-2.45-4.02-2.45 0-2.82 1.91-2.82 3.89V23H8V8.5z" />
          </svg>
        );
      case "twitter":
        return (
      ...
      case "facebook":
        return (
          <svg viewBox="0 0 24 24" width="28" height="28" fill="#1877f2">
            <path d="M22.675 0h-21.35C.595 0 0 .594 0 1.326V22.67c0 .73.595 1.326 1.326 1.326h11.49v-9.86H9.69V10.41h3.126V8.073c0-3.1 1.894-4.79 4.66-4.79 1.325 0 2.46.098 2.79.142v3.24h-1.916c-1.503 0-1.794.715-1.794 1.763v2.31h3.587l-.467 3.726h-3.12V24h6.116C23.405 24 24 23.405 24 22.673V1.326C24 .594 23.405 0 22.675 0z" />
          </svg>
        );
      case "instagram":
        return (
          <svg viewBox="0 0 24 24" width="28" height="28" fill="#E1306C">
            <path d="M7 2C4.24 2 2 4.24 2 7v10c0 2.76 2.24 5 5 5h10c2.76 0 5-2.24 5-5V7c0-2.76-2.24-5-5-5H7zm10 2a3 3 0 0 1 3 3v10a3 3 0 0 1-3 3H7a3 3 0 0 1-3-3V7a3 3 0 0 1 3-3h10zm-5 3a5 5 0 1 0 .001 10.001A5 5 0 0 0 12 7zm0 2.2a2.8 2.8 0 1 1 0 5.6 2.8 2.8 0 0 1 0-5.6zM18 6.5a1 1 0 1 0 0 2 1 1 0 0 0 0-2z" />
          </svg>
        );
      case "youtube":
        return (
      ...
      <div className="px-[31px] pb-[10px] bg-white ">
        <div className="bg-[#3673e1] p-3 mb-8">
          <div className="flex justify-between">
            <div>
              <h2 className="text-white font-sans text-[25px] font-normal relative right-3 shadow-[1px_1px_3px_1px_#18181c] px-2 bg-[#466ad0] mt-[27px] capitalize">
                {name}
              </h2>
...
function RightAbout({ about, name }) {
  return (
    <div className="px-[31px] py-[10px] h-full bg-white overflow-y-auto custom-sidebar">
      <SectionHeading heading="About" sub={`About ${name}`} />
      <p className="whitespace-pre-line mt-4 font-sans text-base leading-6 font-normal text-[#21253A]">
        {about || "—"}
      </p>
    </div>
  );
...
function ChaptersList({
  ...
  return (
    <div className="h-full bg-white">
      <TabsBar tabs={tabs} active={activeTab} onChange={onTabClick} />
        ...
        {!chapters.length ? (
          <div className="text-center py-12">
            ...
            <p className="text-gray-500">
              Start creating your story by adding the first chapter.
            </p>
          </div>
        ) : (
    ...
    <div className="p-6 h-full bg-white overflow-y-auto custom-scrollbar">
      ...
      {!recs.length ? (
        ...
        recs.map((r) => (
          <div
            key={r.id}
            className="bg-[#f1e8d6] p-5 max-w-3xl mb-6 mt-6 shadow-[1px_1px_9px_1px_#afafaf]"
          >
            <div className="flex items-start gap-3 mb-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-[#21253A] text-sm leading-7 whitespace-pre-line my-4 font-normal">
                  {r.content}
                </p>
                {r.skill ? (
                  <p className="text-sm text-gray-600">{r.skill}</p>
                ) : null}
                {r.linkedin_url ? (
                  <a
                    href={r.linkedin_url}
      ...
      <div className="p-6 pt-0">
        <div className="text-center mb-8">
          <h2 className="font-cinzel font-bold leading-tight tracking-wide text-2xl sm:text-3xl">
            ...
            <br />
            <span className="inline-block px-3 uppercase">{name}</span>
          </h2>
        </div>
...
function SocialDetail({ post }) {
  ...
  const isYT =
    lower === "youtube" || /youtu\.be|youtube\.com/.test(post.url || "");
  const ytId = (() => {
    if (!isYT) return null;
    try {
  ...
  return (
    <div className="p-10 h-full bg-white">
      ...
      {isYT && ytId ? (
        ...
        <>
          <p className="text-gray-700 leading-7 whitespace-pre-line mb-4">
            {post.caption}
          </p>
          <a
            href={post.url}
...
export default function DiaryBook({
  ...
  const name =
    ...
    `${executive.user_first_name || ""} ${
      executive.user_last_name || ""
    }`.trim() ||
    "Executive";
  ...
  const [selectedPostId, setSelectedPostId] = useState(
    socialPosts?.[0]?.id || null
  );
  useEffect(() => {
    setSelectedPostId(socialPosts?.[0]?.id || null);
  }, [socialPosts]);
  ...
  const spreads = useMemo(() => {
    ...
    const aboutRight = (
      <RightAbout key="about-right" about={executive.about} name={name} />
    );
    ...
    const socialRight = (
      <SocialDetail
        ...
        post={
          socialPosts?.find((p) => p.id === selectedPostId) || socialPosts?.[0]
        }
      />
    );
  ...
  function flipToTab(tabIndex) {
    ...
    console.log("API available, attempting flip...");

    try {
      api.stopFlip?.();
    } catch {}

    // Use flipNext or flipPrev for animation
    if (tabIndex > activeTab) {
      // Going forward (About -> Recommendations)
      console.log("Flipping forward...");
      api.flipNext?.();
    } else if (tabIndex < activeTab) {
      // Going backward (Recommendations -> About)
      console.log("Flipping backward...");
      api.flipPrev?.();
    } else {
      // Already on the right page, just update state
      console.log("Already on correct page, using turnToPage...");
  ...
  function handleFlip(e) {
    ...
    // Play flip sound only for user-initiated flips (not programmatic)
    playFlipSound();
  ...
  useEffect(() => {
    if (mounted) {
      // Initialize without sound on first load
      setTimeout(() => {
        const api = getApi();
        if (api) {
          api.turnToPage?.(0); // Go to first page without animation
          setActiveTab(0);
        }
      }, 100);
    }
  ...
  const goPrev = () => {
    ...
    const api = getApi();
    if (api) {
      console.log("Flipping previous page...");
      api.flipPrev?.();
  ...
  const goNext = () => {
    ...
    const api = getApi();
    if (api) {
      console.log("Flipping next page...");
      api.flipNext?.();
      setActiveTab(activeTab + 1);
    }
  };
          ...
          <div className="bg-white rounded-md overflow-visible relative">
            <div className="relative book-shadow overflow-visible">
              <div className="mx-auto overflow-hidden">
                <div className="w-[725px] h-[550.25px] flex items-center justify-center">
