...
const DiaryOfTheDay = () => {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_DOMAIN}/api/executives/`);
        const data = await response.json();
        if (response.ok) {
          const diaries = data.data.filter((entry) => entry.labels.includes("diary_of_the_day"));
          const founders = data.data.filter((entry) => entry.labels.includes("founder_of_the_day"));
  ...
  const nextDiary = () => setDiaryIndex((prev) => (prev + 1) % diaryEntries.length);
  const prevDiary = () => setDiaryIndex((prev) => (prev - 1 + diaryEntries.length) % diaryEntries.length);
  const nextFounder = () => setFounderIndex((prev) => (prev + 1) % founderEntries.length);
  const prevFounder = () => setFounderIndex((prev) => (prev - 1 + founderEntries.length) % founderEntries.length);
