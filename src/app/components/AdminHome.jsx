...
const ProgressBar = ({ value = 0, max = 1 }) => {
...
const AdminHome = ({ analytics, loading, error }) => {
  // Accept either full API object or just the .data node
  ...
  const contributors = Number(dataNode?.total_contributors ?? 0);

  // Labels
  const labelsNode = dataNode?.labels ?? {};
  const labelKeys = ['trending', 'featured', 'diary_of_the_day', 'founder_of_the_day', 'popular'];
  const labelsData = labelKeys.map((k) => ({ key: k, value: Number(labelsNode?.[k] ?? 0) }));
  const totalLabels = useMemo(
    () => labelsData.reduce((acc, d) => acc + d.value, 0),
    [labelsData]
  );

  // Top stats (Admin adds Total Contributors)
  const stats = [
    ...
    { name: 'Total Contributors', value: contributors, icon: Users, color: 'bg-blue-500' },
  ];
  ...
  return (
    <div className="p-6">
      ...
      <div className="mb-6">
        ...
        <p className="text-gray-600 mt-1 sm:mt-2">Platform-wide overview of biographies and labels.</p>

        {loading && <div className="mt-3 text-sm text-gray-500">Loading analytics…</div>}
        {error && <div className="mt-3 text-sm text-red-600">{error}</div>}
      </div>

      {/* Top stats — responsive 1→2→3 cols */}
      ...
      <div className="grid grid-cols-1 xs:grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-4 sm:gap-6">
        {labelsData.map(({ key, value }) => {
          const maxVal = Math.max(...labelsData.map((d) => d.value), 1);
          const pct = totalLabels > 0 ? Math.round((value / totalLabels) * 100) : 0;
          const colorClasses = labelColor(key);
          return (
            <div key={key} className={`border ${colorClasses} rounded-lg p-4 sm:p-5 flex flex-col gap-3`}>
              <div className="flex items-center justify-between gap-3">
              ...
              <ProgressBar value={value} max={maxVal} />
              <span className="sr-only">
                {formatLabel(key)}: {value} ({pct}%)
              </span>
            </div>
          );
        })}
      ...
      {!loading && totalLabels === 0 && (
        <div className="mt-6 text-sm text-gray-500">
          No labeled biographies yet. Apply labels like <span className="font-medium">Trending</span> or{' '}
