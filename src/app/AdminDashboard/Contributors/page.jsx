...
const ContributorsPage = () => {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      const storedToken = localStorage.getItem('authToken'); // Fetch token only in the client-side
    ...
    <div className="p-6">
      ...
      <div className="mb-8 flex justify-between items-center">
        ...
        <button
          ...
          className="bg-[#1b1b4a] hover:bg-[#1b1b4a] text-white px-4 py-2 rounded-lg flex items-center space-x-2 disabled:opacity-60 cursor-pointer"
          disabled={creating}
        >
              ...
              {filteredContributors.map((c) => (
                <tr key={c.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium flex justify-end space-x-2">
                    <button onClick={() => openEditModal(c)} className="text-blue-600 hover:text-blue-900 p-1 hover:bg-blue-50 rounded cursor-pointer">
                      <Edit className="h-4 w-4" />
                    </button>
                    <button onClick={() => alert('Delete API not yet integrated')} className="text-red-600 hover:text-red-900 p-1 hover:bg-red-50 rounded cursor-pointer">
